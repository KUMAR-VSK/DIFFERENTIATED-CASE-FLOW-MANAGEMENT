        ...
        <div>
            ...
            {documents.length === 0 ? (
                <div className="text-center py-12">
                    ...
                    <h3 className="mt-2 text-sm font-medium text-surface-900">No documents</h3>
                    <p className="mt-1 text-sm text-surface-500">No documents have been uploaded for this case yet.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {documents.map((doc, index) => (
                        <div key={doc.id || index} className="bg-white dark:bg-surface-800 rounded-lg border border-surface-200 dark:border-surface-700 p-4 hover:shadow-md transition-shadow duration-200 transition-colors">
                            <div className="flex items-start justify-between">
                                <div className="flex items-start">
                                    <div className="bg-blue-100 dark:bg-blue-900/30 rounded-lg p-2 mr-3 transition-colors">
                                        <svg className="h-6 w-6 text-primary-600 dark:text-blue-400" fill="currentColor" viewBox="0 0 20 20">
                                            <path fillRule="evenodd" d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm0 2h12v10H4V5z" clipRule="evenodd" />
                                        </svg>
                                    </div>
                                    <div className="flex-1">
                                        <h3 className="text-sm font-medium text-surface-900 dark:text-white truncate">{doc.originalFileName}</h3>
                            ...
                            <div className="mt-4 flex justify-end gap-3">
                                <button
                                    ...
                                    className="text-primary-600 hover:text-blue-800 text-sm font-medium"
                                >
                                    View
                                </button>
